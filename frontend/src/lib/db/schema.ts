...
export const savedResults = pgTable('saved_results', {
  ...
  projectId: integer('project_id').references(() => researchProjects.id, { onDelete: 'set null' }),
...
export const researchProjects = pgTable('research_projects', {
  ...
  isArchivedIdx: index('research_projects_is_archived_idx').on(table.isArchived),
}));

/**
 * Junction table - links searches to projects (many-to-many)
 */
export const projectSearches = pgTable('project_searches', {
  projectId: integer('project_id').references(() => researchProjects.id, { onDelete: 'cascade' }).notNull(),
  searchId: integer('search_id').references(() => searches.id, { onDelete: 'cascade' }).notNull(),
...
export const userPreferences = pgTable('user_preferences', {
  ...
  defaultEngines: text('default_engines').array().default(['brave', 'duckduckgo']),
...
export const researchProjectsRelations = relations(researchProjects, ({ many }) => ({
  savedResults: many(savedResults),
  projectSearches: many(projectSearches),
}));

export const projectSearchesRelations = relations(projectSearches, ({ one }) => ({
  project: one(researchProjects, {
